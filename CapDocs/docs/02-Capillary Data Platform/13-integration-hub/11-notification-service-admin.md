...
The communication gateway is a device that acts as a communication between internet protocols and telecom. The communication gateways make it easy to add communication services to internet-based applications.
...
Scroll down to Add/Update Gateway page
...
15. In the **Gateway Countries box**, select the countries to which sending messages are applicable
...
1. In the **System Gateway** table, navigate to the gateway which you want to modify and scroll the horizontal bar to the end and click the **edit link** of the respective gateway. The details of the gateway will appear on the **Add/Update** section
...
The gateways that are added to the organization have to be configured to be used for the organization. To configure the gateway to use by the organization use the following instructions.
...
6. In the **Gateway Weight box**, restrict the messages sending through the gateway in terms of percentage of total messages.
...
You can restrict the type of messages (bulk, transaction, or default) that can be sent through a gateway. This allows setting up a gateway only for sending particular types of messages. For instance, a gateway can be
...
For example, if the total number of messages to be sent for a campaign are 1000 and if 80 is set in Gateway Weight, then only 800 messages (80% of 1000) will be sent through this gateway.
...
#### To generate message delivery report
...
3. In the **Start Date** and **End Date** fields and set the time period for which the delivery report has to be generated.
...
You can unsubscribe a customer's mobile numbers or email id from the organization. This is commonly called DND (Do Not Disturb). Any mobile number or email id once unsubscribed from the organization will not receive any communication messages from the organization.
...
2. In the **Resubscription** field use the following steps to resume getting messages from the organization.
...
![a73526f Screenshot from 2013 06 20 15 41 50](https://files.readme.io/a73526f-Screenshot_from_2013-06-20_15_41_50.png)
...
![192d04e SMS Gateway](https://files.readme.io/192d04e-SMS_Gateway.png)
...
![6c4fd0d email Org](https://files.readme.io/6c4fd0d-email_Org.png)

### Campaign wise message summary report

This page allows you to generate a report of the status of messages sent by a particular campaign of an organization.

![3d093b4 SMS Campaign](https://files.readme.io/3d093b4-SMS_Campaign.png)

> 📘 * A report summary will be generated only when you select the date range and click Submit from the Date Range box
> * Access to dashboards and reports is restricted. Hence, everyone cannot access this page 
